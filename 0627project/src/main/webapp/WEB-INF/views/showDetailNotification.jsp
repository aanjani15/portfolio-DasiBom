		...
		<table class = "post-detail-table">
			<tr>
				<td colspan="4" class="btn-row">
					<input type="submit" value="공지 목록" onclick="submitTo('goNotification.do')" class="post-detail-button" />

					<c:if test="${sessionScope.user_id != null && sessionScope.user_id == post.user_id}">
						<input type="submit" value="수정하기" onclick="submitTo('goWriteNotification.do?post_id=${post.post_id}')" class="post-detail-button" />
						<input type="submit" value="삭제하기" onclick="submitTo('deleteNotification.do?post_id=${post.post_id}')" class="post-detail-button" />
					</c:if>
				</td>
			</tr>
