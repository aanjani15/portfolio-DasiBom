...
<body class="write-body">
	...
	<div class="write-content-wrapper">
		<h1 class="write-title">이벤트 게시글 작성</h1>
		<hr class="write-hr" />

		<form id="myForm" method="post">
			<div class="write-button-row">
				<input type="button" value="모두보기" onclick="location.href = 'goEvent.do'" />

				<c:choose>
					<c:when test="${empty post.post_id}">
						<input type="button" id="writebtn" value="작성하기" onclick="submitTo('insertEvent.do')" />
					</c:when>
					<c:otherwise>
						<input type="hidden" name="post_id" value="${post.post_id}" />
						<input type="button" id="updatebtn" value="수정하기" onclick="submitTo('updateEvent.do')" />
